...
function finish(ok){clearInterval(timer);progress(100,ok?'✅ Analyse terminée':'⚠️ Analyse interrompue',ok?'Résultat prêt.':'Le service est momentanément indisponible.','#16a34a');if(!ok)$('vlc-safe-fill').style.background='#dc2626';setTimeout(()=>{const p=$('vlc-safe-progress');if(p)p.style.display='none'},ok?2500:8000);active=false}
