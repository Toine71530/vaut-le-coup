...
/* Exactly three thumbnails on one row, including narrow phones. */
.previews{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;grid-auto-flow:row!important;gap:10px!important;margin-top:12px!important;width:100%!important;overflow:visible!important;flex-wrap:nowrap!important}
.thumb{position:relative!important;width:100%!important;min-width:0!important;max-width:none!important;height:auto!important;aspect-ratio:1/1!important;flex:none!important;margin:0!important}
.thumb img{display:block!important;width:100%!important;height:100%!important;object-fit:cover!important;border-radius:18px!important;border:1px solid #ddd!important}
.remove{z-index:3!important}
...
.analysis-progress-track{height:14px;background:#e1e6eb;border-radius:99px;overflow:hidden;box-shadow:inset 0 1px 2px #00000012}
.analysis-progress-bar{height:100%;width:5%;border-radius:99px;background:#3b82f6;transition:width .8s ease,background .5s ease;box-shadow:0 1px 4px #00000020}
.analysis-progress-bar.stage-1{background:#3b82f6}
.analysis-progress-bar.stage-2{background:#2563eb}
.analysis-progress-bar.stage-3{background:#f59e0b}
.analysis-progress-bar.stage-4{background:#f59e0b}
.analysis-progress-bar.stage-5{background:#16a34a}
.analysis-progress-note{font-size:14px;color:#68727d;margin-top:9px}
</style></head>`);
        html = html.replace('<div id="error"></div>', '<div id="error"></div><div id="analysis-progress" class="analysis-progress" aria-live="polite"><div id="analysis-progress-title" class="analysis-progress-title">Analyse des photos…</div><div class="analysis-progress-track"><div id="analysis-progress-bar" class="analysis-progress-bar stage-1"></div></div><div id="analysis-progress-note" class="analysis-progress-note">Lecture des informations visibles</div></div>');
        html = html.replace("</body>", `<script>(function(){const go=document.getElementById('go'),box=document.getElementById('analysis-progress'),bar=document.getElementById('analysis-progress-bar'),title=document.getElementById('analysis-progress-title'),note=document.getElementById('analysis-progress-note'),error=document.getElementById('error');if(!go||!box)return;let timer=null,step=0;const stages=[['Analyse des photos…','Lecture des informations visibles',20],['Extraction des données…','Identification du véhicule, du prix et du kilométrage',40],['Vérification…','Contrôle des informations détectées',60],['Recherche du marché…','Recherche de véhicules réellement comparables',80],['Finalisation…','Préparation du résultat',95]];function setStage(i){const s=stages[Math.min(i,stages.length-1)];title.textContent=s[0];note.textContent=s[1];bar.style.width=s[2]+'%';bar.className='analysis-progress-bar stage-'+(i+1)}function start(){box.classList.add('active');step=0;setStage(0);clearInterval(timer);timer=setInterval(()=>{if(step<stages.length-1){step++;setStage(step)}},1600)}function finish(){clearInterval(timer);bar.className='analysis-progress-bar stage-5';bar.style.width='100%';title.textContent='Analyse terminée';note.textContent='Résultat prêt';setTimeout(()=>box.classList.remove('active'),900)}document.addEventListener('click',e=>{if(e.target===go)start()},true);new MutationObserver(()=>{if(error&&error.textContent.trim()){box.classList.remove('active');clearInterval(timer)}const market=document.getElementById('market');if(market&&!market.hidden&&market.innerText.includes('Comparables'))finish()}).observe(document.body,{subtree:true,childList:true,characterData:true});})();</script></body>`);
        res.status(up.statusCode || 200).set({"content-type":"text/html; charset=utf-8","cache-control":"no-store, no-cache, must-revalidate, proxy-revalidate"}).send(Buffer.from(html));
        return;
      }
