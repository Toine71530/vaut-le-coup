...
/* Mobile: always keep exactly three photo thumbnails on one row. */
.previews{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:12px;width:100%;overflow:visible!important}
.thumb{position:relative!important;width:100%!important;min-width:0!important;max-width:none!important;height:auto!important;aspect-ratio:1/1!important;flex:none!important}
.thumb img{display:block;width:100%;height:100%;object-fit:cover;border-radius:18px;border:1px solid #ddd}
.remove{z-index:2}
...
.analysis-progress-note{font-size:14px;color:#68727d;margin-top:9px}
        ...
        html = html.replace("</body>", `<script>(function(){const go=document.getElementById('go'),box=document.getElementById('analysis-progress'),bar=document.getElementById('analysis-progress-bar'),title=document.getElementById('analysis-progress-title'),note=document.getElementById('analysis-progress-note'),error=document.getElementById('error');if(!go||!box)return;let timer=null,step=0;const stages=[['Analyse des photos…','Lecture des informations visibles',20],['Extraction des données…','Identification du véhicule, du prix et du kilométrage',40],['Vérification…','Contrôle des informations détectées',60],['Recherche du marché…','Recherche de véhicules réellement comparables',80],['Finalisation…','Préparation du résultat',95]];function setStage(i){const s=stages[Math.min(i,stages.length-1)];title.textContent=s[0];note.textContent=s[1];bar.style.width=s[2]+'%';bar.className='analysis-progress-bar stage-'+(i+1)}function start(){if(!go||go.disabled)return;box.classList.add('active');step=0;setStage(0);clearInterval(timer);timer=setInterval(()=>{if(step<stages.length-1){step++;setStage(step)}},1800)}function finish(){clearInterval(timer);bar.className='analysis-progress-bar stage-5';bar.style.width='100%';title.textContent='Analyse terminée';note.textContent='Résultat prêt';setTimeout(()=>box.classList.remove('active'),700)}document.addEventListener('click',e=>{if(e.target===go){setTimeout(start,0)}},true);new MutationObserver(()=>{if(error&&error.textContent.trim()){box.classList.remove('active');clearInterval(timer)}const market=document.getElementById('market');if(market&&!market.hidden&&market.innerText.trim()&&market.innerText.includes('Comparables'))finish()}).observe(document.body,{subtree:true,childList:true,characterData:true});})();</script></body>`);
        res.status(up.statusCode || 200).set("content-type", "text/html; charset=utf-8").send(Buffer.from(html));
        return;
      }
