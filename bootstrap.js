// Production bootstrap with resilient Gemini fallback and safe HTTP proxying.
...
async function resilientGeminiFetch(input, init) {
  ...
  // Use stable production models only. If capacity is temporarily unavailable,
  // retry the primary and then use another stable Flash model.
  const models = [
    "gemini-3.6-flash",
    "gemini-3.6-flash",
    "gemini-3.7-flash",
...
// Public proxy: buffer the complete upstream response before setting headers.
// This avoids ERR_HTTP_HEADERS_SENT and truncated JSON responses.
